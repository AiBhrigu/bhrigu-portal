...
function SemanticVisual({ locale, visual }: { locale: BtcCleanLocale; visual: BtcCleanSemanticVisual }) {
  ...
  const stateLabel = visual.state === "CONFIRMATION" ? (ru ? "Подтверждение" : "Confirmation")
    : visual.state === "DIVERGENCE" ? (ru ? "Расхождение" : "Divergence")
      : visual.state === "LIMITED" ? (ru ? "Ограничено" : "Limited")
        : visual.state === "TEMPORAL" ? (ru ? "Временной контекст" : "Temporal")
          : (ru ? "Ожидание" : "Expectation");
  return <div className="cleanSemanticVisual" data-semantic-visual={visual.kind} data-semantic-state={visual.state} aria-label={ru ? "Смысловой маркер поля" : "Field meaning marker"}>
    ...
    <span className="cleanVisualLabel">{stateLabel}</span>
  </div>;
}
