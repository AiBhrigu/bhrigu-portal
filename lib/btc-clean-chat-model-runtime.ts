...
export async function runBtcCleanChatModel(input: { locale: BtcCleanLocale; question: string; priorTurns?: BtcCleanPriorTurn[]; fieldContext?: BtcResearchFieldModelContext }): Promise<BtcCleanChatResponse> {
  ...
  const visualOnly = synthesis.status === "OUTPUT_LIMIT" && semanticVisual?.native?.type === "CURRENT_TO_GENESIS_MATRIX";
  ...
  return {
    ...
    answer: synthesis.status === "COMPLETE" ? synthesis.answer : visualOnlyOutputLimitNotice(input.locale),
