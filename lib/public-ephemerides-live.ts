...
import {
  buildPublicEphemeridesMonth,
  preferredPublishedMonth,
  PUBLIC_EPHEMERIDES_YEAR,
  type PublicAstroLocale,
} from "./public-ephemerides-v1";
...
function nextPublishedTurningPoint(locale:PublicAstroLocale,now:Date){
  if(now.getUTCFullYear()!==PUBLIC_EPHEMERIDES_YEAR)return null;
  const utcDate=now.toISOString().slice(0,10);
  const startMonth=now.getUTCMonth()+1;
  const candidates:Array<any>=[];
  for(let month=startMonth;month<=12;month+=1){
    const published=buildPublicEphemeridesMonth(locale,month) as any;
    for(const station of published.stations??[]){
      if(station.date<utcDate)continue;
      candidates.push({date:station.date,kind:"station",body:station.body,name:station.name,motion:station.motion,motionLabel:station.motionLabel,source:{authority:"PUBLISHED_MONTHLY_ASTRO_EVIDENCE",boundary:"CONTEXT_ONLY_NOT_DAILY_DERIVATION",month}});
    }
    for(const ingress of published.ingresses??[]){
      if(ingress.date<utcDate)continue;
      candidates.push({date:ingress.date,kind:"ingress",body:ingress.body,name:ingress.name,sign:ingress.sign,signLabel:ingress.signLabel,source:{authority:"PUBLISHED_MONTHLY_ASTRO_EVIDENCE",boundary:"CONTEXT_ONLY_NOT_DAILY_DERIVATION",month}});
    }
  }
  candidates.sort((a,b)=>a.date.localeCompare(b.date)||String(a.kind).localeCompare(String(b.kind))||String(a.body).localeCompare(String(b.body)));
  return candidates[0]??null;
}

function projectDailyRow(locale:PublicAstroLocale,row:DailyRow,now:Date){
  const bodies=Object.entries(row.bodies).map(([name,state])=>{const key=name.toLowerCase(),longitude=Number(state.longitude_deg),speed=Number(state.longitude_speed_deg_per_day);return {key,name:bodyName(key,locale),longitude:Number(longitude.toFixed(4)),speed:Number(speed.toFixed(5)),retrograde:Boolean(state.retrograde),position:formatZodiacPosition(longitude,locale)};});
