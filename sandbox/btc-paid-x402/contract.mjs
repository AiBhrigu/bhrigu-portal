...
export function hmacReference(value, key = OFFLINE_FIXTURE_EVIDENCE_KEY) {
  ...
  if (process.env.X402_SANDBOX_LIVE_EXECUTION_ACK === 'BASE_SEPOLIA_TEST_USDC_ONLY' && key === OFFLINE_FIXTURE_EVIDENCE_KEY) {
    throw new Error('OFFLINE_EVIDENCE_KEY_FORBIDDEN_IN_LIVE_MODE');
  }
...
export function paymentFingerprint(tuple = {}) {
  return sha256(canonical({
    protocol_version: tuple.protocolVersion, scheme: tuple.scheme, network: tuple.network,
    ...
    product_id: tuple.productId, http_method: tuple.method, logical_operation: tuple.operation,
    environment: tuple.environment,
  }));
}
export function trustedTuple(overrides = {}, expectedPayTo = TRUSTED.fixturePayTo) {
  return { ...TRUSTED, payTo: expectedPayTo, ...overrides };
}
export function buildRequirement(payTo) {
  ...
  return {
    ...
    accepts: [{
      ...
      amount: TRUSTED.amountAtomic, payTo, maxTimeoutSeconds: 60,
      extra: { name: TRUSTED.assetSymbol, version: '2' },
    }],
    extensions: { [PAYMENT_IDENTIFIER]: declarePaymentIdentifierExtension(true) },
  };
}
export function public402(payTo = TRUSTED.fixturePayTo) {
  return { httpStatus: 402, state: STATES.REQUIREMENT_ISSUED, requirement: buildRequirement(payTo) };
}
...
export function validateTrustedTuple(tuple = {}, expectedPayTo = TRUSTED.fixturePayTo) {
  const checks = [
    ['protocolVersion', TRUSTED.protocolVersion], ['scheme', TRUSTED.scheme], ['network', TRUSTED.network],
    ['asset', TRUSTED.asset], ['amountAtomic', TRUSTED.amountAtomic], ['payTo', expectedPayTo],
    ['resourceId', TRUSTED.resourceId], ['productId', TRUSTED.productId],
    ['environment', TRUSTED.environment], ['operation', TRUSTED.operation], ['method', TRUSTED.method],
  ];
...
export function validateOfficialPaymentPayload(paymentPayload, expectedPayTo) {
  ...
  const actual = paymentPayload?.accepted || {};
  const expected = buildRequirement(expectedPayTo).accepts[0];
  const checks = [
    ['X402VERSION', paymentPayload?.x402Version, TRUSTED.protocolVersion], ['SCHEME', actual.scheme, expected.scheme],
...
export function normalizeSettlement({ paymentIdentifier, verification, settlement,
  expectedPayTo = TRUSTED.fixturePayTo, evidenceKey = OFFLINE_FIXTURE_EVIDENCE_KEY,
  ...
  const tuple = validateTrustedTuple(settlement.tuple, expectedPayTo);
  if (!tuple.ok) throw new Error(tuple.code);
  if (!HEX_32.test(settlement.transaction)) throw new Error('MALFORMED_TRANSACTION_REFERENCE');
  if (typeof settlement.eventId !== 'string' || settlement.eventId.length < 8) throw new Error('MALFORMED_EVENT_REFERENCE');
  if (typeof settlement.receipt !== 'string' || settlement.receipt.length < 8) throw new Error('MALFORMED_RECEIPT_REFERENCE');
  return Object.freeze({
    provider: TRUSTED.provider, protocol_version: TRUSTED.protocolVersion, environment: TRUSTED.environment,
    scheme: TRUSTED.scheme, network: TRUSTED.network,
    event_id: hmacReference(settlement.eventId, evidenceKey),
    payment_identifier_hash: hmacReference(paymentIdentifier, evidenceKey),
    receipt_reference_hash: hmacReference(settlement.receipt, evidenceKey),
    transaction_reference_hash: hmacReference(settlement.transaction.toLowerCase(), evidenceKey),
    product_id: TRUSTED.productId, resource_id: TRUSTED.resourceId, amount_atomic: TRUSTED.amountAtomic,
    currency_or_asset: TRUSTED.asset, pay_to_hash: hmacReference(expectedPayTo.toLowerCase(), evidenceKey),
    status: 'settled', verified_at: verification.verifiedAt || now, settled_at: settlement.settledAt || now,
  });
}

export async function processPayment(input, {
  ledger, facilitator, clock = () => Date.now(), preflight = async () => true,
  expectedPayTo = TRUSTED.fixturePayTo, evidenceKey = OFFLINE_FIXTURE_EVIDENCE_KEY,
  requirement = buildRequirement(expectedPayTo),
} = {}) {
  if (!input?.paymentHeader) return public402(expectedPayTo);
  const envelope = validateEnvelope(input);
  if (!envelope.ok) return envelope;
  if (input.paymentHeader.officialPayload) {
    const official = validateOfficialPaymentPayload(input.paymentHeader.officialPayload, expectedPayTo);
  ...
  const tuple = validateTrustedTuple(input.paymentHeader.tuple, expectedPayTo);
  ...
  try {
    ...
    try { await ledger.cancelReservation(input.paymentIdentifier, 'PREFLIGHT_EXCEPTION'); } catch {}
    return reject(503, 'PROTECTED_PREFLIGHT_FAILED', STATES.REVIEW_REQUIRED);
  }
  ...
  try { settlement = await facilitator.settle(input.paymentHeader, requirement); }
    ...
    if (['SETTLE_TIMEOUT','SETTLE_UNKNOWN'].includes(error.code)) {
  ...
  let finalization;
  try {
    finalization = await ledger.finalizeSettlement({ paymentIdentifier: input.paymentIdentifier, normalized, payloadHash,
      receiptHash: normalized.receipt_reference_hash, transactionHash: normalized.transaction_reference_hash,
  ...
  if (finalization.kind === 'ledger-failure') {
    try { await ledger.markReconciliation(input.paymentIdentifier, 'SETTLED_LEDGER_FINALIZATION_FAILED'); } catch {}
    return reject(202, 'ENTITLEMENT_WRITE_FAILED', STATES.ENTITLEMENT_WRITE_FAILED);
  }

  let entitlement;
  try { entitlement = await ledger.issueEntitlement({ paymentIdentifier: input.paymentIdentifier, normalized,
    successfulTurns: TRUSTED.successfulTurns, activationWindowSeconds: TRUSTED.activationWindowSeconds,
    activeLifetimeSeconds: TRUSTED.activeLifetimeSeconds, now: clock() }); }
  ...
  const result = { httpStatus: 200, state: STATES.ENTITLEMENT_ISSUED,
    entitlement: entitlement.publicView, payment: normalized, replay: false };
...
export async function refundPayment({ paymentIdentifier, ledger, facilitator, clock = () => Date.now() }) {
  ...
  try {
    ...
    const result = await ledger.completeRefund(paymentIdentifier, {
      transactionReferenceHash: hmacReference(refund.transaction.toLowerCase(), facilitator.evidenceKey || OFFLINE_FIXTURE_EVIDENCE_KEY),
