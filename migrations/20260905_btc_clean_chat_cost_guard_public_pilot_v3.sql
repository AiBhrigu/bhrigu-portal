-- Public pilot V3: raise only global money circuit breakers; preserve all rate, concurrency, replay, privacy, and settlement laws.
BEGIN;
...
BEGIN
  ...
  IF v_cost+p_reservation_micros > 2000000 THEN
  ...
  IF v_cost+p_reservation_micros > 6000000 THEN
    v_budget_limited := TRUE;
  ...
  IF v_cost+p_reservation_micros > 25000000 THEN
    v_budget_limited := TRUE;
...
BEGIN
  ...
  SELECT COALESCE(SUM(btc_clean_chat_guard_effective_cost(state,reservation_micros,settled_micros)),0)::bigint,MIN(started_at)
    INTO v_cost,v_oldest FROM btc_clean_chat_cost_admissions WHERE started_at > p_now-INTERVAL '1 hour';
  IF v_cost+v_delta > 2000000 THEN v_budget_limited := TRUE; IF v_oldest IS NOT NULL THEN v_retry := GREATEST(v_retry,CEIL(EXTRACT(EPOCH FROM (v_oldest+INTERVAL '1 hour'-p_now)))::int); END IF; END IF;

  SELECT COALESCE(SUM(btc_clean_chat_guard_effective_cost(state,reservation_micros,settled_micros)),0)::bigint,MIN(started_at)
    INTO v_cost,v_oldest FROM btc_clean_chat_cost_admissions WHERE started_at > p_now-INTERVAL '24 hours';
  IF v_cost+v_delta > 6000000 THEN v_budget_limited := TRUE; IF v_oldest IS NOT NULL THEN v_retry := GREATEST(v_retry,CEIL(EXTRACT(EPOCH FROM (v_oldest+INTERVAL '24 hours'-p_now)))::int); END IF; END IF;

  SELECT COALESCE(SUM(btc_clean_chat_guard_effective_cost(state,reservation_micros,settled_micros)),0)::bigint
    INTO v_cost FROM btc_clean_chat_cost_admissions
    WHERE started_at >= date_trunc('month',p_now) AND started_at < date_trunc('month',p_now)+INTERVAL '1 month';
  IF v_cost+v_delta > 25000000 THEN v_budget_limited := TRUE; v_retry := GREATEST(v_retry,CEIL(EXTRACT(EPOCH FROM (date_trunc('month',p_now)+INTERVAL '1 month'-p_now)))::int); END IF;
