...
import {
  ...
  type PrivateAccessDeliveryState,
...
type Props = {
  generatedAt: string;
  record: ReviewRecord;
  deliveryState: PrivateAccessDeliveryState;
};
...
export const getServerSideProps: GetServerSideProps<Props> = async ({
  ...
  try {
    ...
    const deliveryState = await getPrivateAccessDeliveryState(requestId);
...
function MonoBlock({ children }: { children: React.ReactNode }) {
  return <pre className="monoBlock">{children}</pre>;
}
      ...
      <main>
        <div className="shell">
          <header>
            ...
            <span>
              status: {record.status || "unknown"} · submitted:{" "}
          ...
          <section className="metrics">
            <Metric
              ...
              value={deliveryState.status}
            />
            <Metric
              label="operator sent"
              value={deliveryState.operator.sent ? "yes" : "no"}
            />
            <Metric
              label="client sent"
              value={deliveryState.client.sent ? "yes" : "no"}
            />
            <Metric
              label="intake score"
              value={record.intake_score ?? "n/a"}
            />
            <Metric
              label="priority band"
              value={record.priority_band || "n/a"}
            />
            <Metric
              label="route hint"
              value={record.route_hint || "n/a"}
            />
            <Metric
              label="signal class"
              value={
                record.triage?.signal_class ||
                record.freyPreview?.signal_class ||
                "n/a"
              }
            />
          </section>

          {deliveryState.status === "reconciliation_required" ? (
            <section className="reconciliation">
              <strong>Delivery reconciliation required</strong>
              <p>
                At least one delivery channel was durably reserved without a
                matching sent marker. Automatic resend is blocked to prevent
                duplicate messages.
              </p>
            </section>
          ) : null}
          ...
          <section className="card">
            ...
            <dl>
              <div>
                <dt>Name</dt>
                <dd>{record.request.name}</dd>
              </div>
              <div>
                <dt>Email</dt>
                <dd>{record.request.email}</dd>
              </div>
              <div>
                <dt>Subject</dt>
                <dd>{record.request.subjectType}</dd>
              </div>
            </dl>
            <h3>Main question</h3>
          ...
          <section className="card">
            ...
            <MonoBlock>
              {JSON.stringify(record.operatorPacket ?? null, null, 2)}
            </MonoBlock>
          </section>

          <section className="boundary">
            This capability is private, non-indexed, non-cacheable, and bound
            to one immutable request. Close the tab when review is complete.
          </section>
        </div>
      </main>
      <style jsx>{`
        ...
        main {
          ...
          background: radial-gradient(
              circle at top,
              rgba(215, 182, 111, 0.08),
              transparent 28%
            ),
            linear-gradient(180deg, #07111b, #080b12);
        }
        .shell {
          max-width: 1080px;
          margin: 0 auto;
        ...
        .reconciliation {
          ...
          background: rgba(114, 58, 11, 0.18);
        }
