  ...
  return (
    <>
        ...
        <>
          <section className="freyHero" data-frey-personal-offer="FREY_COSMOGRAPHIC_READING_PASSPORT_USD79_V0_1">
            <div className="freyHeroCopy">
              ...
              <div className="freyHeroActions">
                <a className="primary readingCta" href="#cosmographic-application"><span>{f.primaryCta}</span><b aria-hidden="true">→</b></a>
                <Link className="freyTextLink" href={`/guide/frey?lang=${locale}`}>{f.guideCta}</Link>
              </div>
            </div>

            <div className="fieldPlate" aria-label={f.passportTitle} data-frey-phi-field="PHI_COSMOGRAPHIC_FIELD_V0_1">
              ...
              <div className="fieldOrbit" aria-hidden="true">
                ...
                <span className="orbit orbitD" />
                <span className="orbit orbitE" />
                <span className="orbitTrail trailA" />
                <span className="orbitTrail trailB" />
                ...
                <span className="fieldNode nodeD" />
                <span className="fieldNode nodeE" />
          ...
          <section className="guideBridge">
            ...
            <div className="guideLinks">
              <Link className="methodCta" href={`/guide/frey?lang=${locale}`}>{f.guideCta}</Link>
          ...
          <section className="applicationScene">
            <div className="applicationCopy">
              ...
              <h2>{f.humanTitle}</h2>
              <p>{f.humanLead}</p>
              <a className="applicationCta inline" href={freyMailto}><span>{f.applicationCta}</span><b aria-hidden="true">→</b></a>
            </div>
          ...
          <section className="finalCta freyFinal">
            ...
            <a className="applicationCta" href={freyMailto}><span>{f.applicationCta}</span><b aria-hidden="true">→</b></a>
          </section>
        </>
      ) : (
        <>
      ...
      <style jsx>{`
        ...
        .q[data-access-surface="FREY_COSMOGRAPHIC_READING_PASSPORT_V0_1"]{max-width:1320px}
        ...
        .freyHero{position:relative;display:grid;grid-template-columns:minmax(0,.92fr) minmax(0,1.08fr);min-height:640px;border:1px solid rgba(213,184,109,.22);border-radius:30px;overflow:hidden;background:radial-gradient(circle at 11% 18%,rgba(213,184,109,.12),transparent 34%),radial-gradient(circle at 84% 28%,rgba(98,201,230,.10),transparent 31%),linear-gradient(145deg,rgba(10,13,22,.99),rgba(4,7,13,.98));box-shadow:0 26px 90px rgba(0,0,0,.28)}
        .freyHero::before{content:"";position:absolute;inset:0;pointer-events:none;background:linear-gradient(110deg,transparent 0 42%,rgba(213,184,109,.025) 52%,transparent 64%);mix-blend-mode:screen}
        .freyHero::after{content:"";position:absolute;inset:auto 0 0 0;height:1px;background:linear-gradient(90deg,transparent,rgba(213,184,109,.72),rgba(98,201,230,.38),transparent)}
        .freyHeroCopy{position:relative;z-index:3;min-width:0;display:flex;flex-direction:column;justify-content:center;padding:68px 54px 60px}.freyHeroCopy h1{max-width:100%;font-size:clamp(44px,4.55vw,68px);line-height:.99;overflow-wrap:normal;word-break:normal;hyphens:none}.freyHeroCopy .lead{max-width:610px;font-size:18px}.freyHeroCopy .signal{max-width:620px;padding-top:18px;border-top:1px solid rgba(213,184,109,.16);color:rgba(246,242,228,.76)}
        .freyHeroActions{display:flex;align-items:center;gap:22px;margin-top:26px}.freyTextLink{color:rgba(247,248,250,.72);text-decoration-color:rgba(213,184,109,.38);text-underline-offset:5px;font-size:13px}.readingCta,.applicationCta,.methodCta{box-sizing:border-box;text-decoration:none;transition:transform .2s ease,box-shadow .2s ease,border-color .2s ease,background .2s ease}.readingCta{position:relative;display:inline-flex;align-items:center;justify-content:center;gap:24px;min-width:224px;min-height:52px;margin:0;padding:0 24px;border:1px solid rgba(255,232,159,.76);border-radius:999px;background:linear-gradient(100deg,#d8b85e 0%,#f3dc8e 52%,#d3a942 100%);color:#141109!important;font:800 12px/1 ui-monospace,SFMono-Regular,Menlo,monospace;letter-spacing:.10em;text-transform:uppercase;box-shadow:0 0 0 1px rgba(213,184,109,.12),0 0 24px rgba(232,190,82,.25),0 0 54px rgba(232,190,82,.16)}.readingCta::before{content:"";position:absolute;inset:-10px -18px;border-radius:999px;background:radial-gradient(ellipse at center,rgba(239,199,92,.22),transparent 68%);filter:blur(8px);z-index:-1}.readingCta b,.applicationCta b{font-size:18px;font-weight:400}.readingCta:hover{transform:translateY(-1px);box-shadow:0 0 0 1px rgba(213,184,109,.18),0 0 32px rgba(244,202,95,.36),0 0 72px rgba(244,202,95,.20)}
        .fieldPlate{position:relative;display:flex;min-width:0;min-height:640px;flex-direction:column;justify-content:space-between;padding:32px 34px 30px;border-left:1px solid rgba(213,184,109,.16);background:radial-gradient(circle at 50% 49%,rgba(213,184,109,.10),transparent 18%),radial-gradient(circle at 78% 58%,rgba(98,201,230,.10),transparent 20%),radial-gradient(circle at 26% 35%,rgba(213,184,109,.07),transparent 22%),linear-gradient(180deg,rgba(11,18,30,.96),rgba(4,8,14,.98));overflow:hidden}.fieldPlate::before{content:"";position:absolute;inset:-15%;pointer-events:none;background:radial-gradient(circle at 18% 22%,rgba(255,255,255,.48) 0 1px,transparent 1.5px),radial-gradient(circle at 68% 18%,rgba(98,201,230,.44) 0 1px,transparent 1.5px),radial-gradient(circle at 76% 74%,rgba(213,184,109,.48) 0 1px,transparent 1.5px),radial-gradient(circle at 32% 78%,rgba(255,255,255,.34) 0 1px,transparent 1.5px);background-size:73px 71px,97px 89px,119px 103px,137px 121px;opacity:.34;transform:rotate(7deg)}.fieldPlate::after{content:"";position:absolute;inset:12% 4% 8%;pointer-events:none;background:radial-gradient(ellipse at 50% 50%,rgba(98,201,230,.05),transparent 58%);border-radius:50%;filter:blur(8px)}.plateTopline,.requestSheetTop{position:relative;z-index:3;display:flex;justify-content:space-between;color:rgba(224,194,111,.85);font:600 10px/1.4 ui-monospace,SFMono-Regular,Menlo,monospace;letter-spacing:.15em}.fieldOrbit{position:relative;z-index:2;width:min(460px,88%);aspect-ratio:1;margin:8px auto 12px}.orbit,.axis,.fieldNode,.fieldCore,.fieldWord,.orbitTrail{position:absolute}.orbit{inset:50%;border:1px solid rgba(213,184,109,.22);border-radius:50%;transform:translate(-50%,-50%)}.orbitA{width:92%;height:92%;box-shadow:inset 0 0 38px rgba(213,184,109,.025)}.orbitB{width:68%;height:68%;border-color:rgba(98,201,230,.26)}.orbitC{width:39%;height:39%;border-color:rgba(213,184,109,.42)}.orbitD{width:100%;height:54%;border-color:rgba(213,184,109,.30);transform:translate(-50%,-50%) rotate(31deg)}.orbitE{width:96%;height:47%;border-color:rgba(98,201,230,.24);transform:translate(-50%,-50%) rotate(-38deg)}.orbitTrail{left:8%;right:8%;top:50%;height:48%;border-top:1px dotted rgba(213,184,109,.32);border-radius:50%;transform-origin:center}.trailA{transform:translateY(-50%) rotate(17deg)}.trailB{transform:translateY(-50%) rotate(-27deg);border-color:rgba(98,201,230,.28)}.axis{background:linear-gradient(90deg,transparent,rgba(255,255,255,.17),transparent)}.axisX{left:1%;right:1%;top:50%;height:1px}.axisY{top:1%;bottom:1%;left:50%;width:1px;background:linear-gradient(180deg,transparent,rgba(255,255,255,.17),transparent)}.fieldCore{left:50%;top:50%;transform:translate(-50%,-50%);display:grid;place-items:center;width:64px;height:64px;border:1px solid rgba(232,197,98,.78);border-radius:50%;background:radial-gradient(circle at 50% 40%,rgba(213,184,109,.12),rgba(5,8,15,.96) 62%);color:#f1cf70;font:400 30px/1 Georgia,serif;box-shadow:0 0 0 13px rgba(213,184,109,.035),0 0 46px rgba(226,185,74,.28),0 0 90px rgba(226,185,74,.12);animation:phiPulse 4.8s ease-in-out infinite}.fieldNode{width:9px;height:9px;border-radius:50%;background:#e0bd61;box-shadow:0 0 10px rgba(213,184,109,.75),0 0 28px rgba(213,184,109,.38)}.nodeA{left:18%;top:34%}.nodeB{right:13%;top:54%;background:#62c9e6;box-shadow:0 0 10px rgba(98,201,230,.8),0 0 28px rgba(98,201,230,.38)}.nodeC{left:44%;bottom:6%}.nodeD{left:31%;top:16%;width:12px;height:12px}.nodeE{right:28%;bottom:22%;width:7px;height:7px;background:#62c9e6}.fieldWord{color:rgba(255,255,255,.54);font:600 9px/1.3 ui-monospace,SFMono-Regular,Menlo,monospace;letter-spacing:.18em;text-shadow:0 0 18px rgba(255,255,255,.08)}.fieldWordA{left:1%;top:48%}.fieldWordB{right:0;top:22%}.fieldWordC{right:2%;bottom:17%}.fieldWordD{left:3%;bottom:14%}.passportPrice{position:relative;z-index:3;display:grid;gap:5px;padding-top:20px;border-top:1px solid rgba(255,255,255,.09)}.passportPrice span,.passportPrice small{color:rgba(255,255,255,.52);font:600 9px/1.45 ui-monospace,SFMono-Regular,Menlo,monospace;letter-spacing:.11em}.passportPrice strong{color:#fff;font:500 42px/1 Georgia,serif;text-shadow:0 0 26px rgba(213,184,109,.12)}
        .passportBand,.passportSpecimen,.entryScene,.freyContextScene,.guideBridge,.applicationScene,.trustLayer,.processScene{margin-top:18px;border:1px solid rgba(255,255,255,.085);border-radius:22px;background:linear-gradient(145deg,rgba(255,255,255,.014),rgba(255,255,255,.006));box-shadow:0 18px 50px rgba(0,0,0,.10)}
        .passportBand{display:grid;grid-template-columns:minmax(0,1.05fr) minmax(300px,.95fr);gap:40px;align-items:center;padding:38px;background:radial-gradient(circle at 88% 50%,rgba(98,201,230,.045),transparent 30%),linear-gradient(145deg,rgba(255,255,255,.014),rgba(255,255,255,.006))}.passportBand h2{font-size:clamp(34px,4vw,50px)}.subjectRail{display:grid;gap:18px;padding:24px 0}.subjectRail span{color:rgba(247,248,250,.72);font:600 11px/1.7 ui-monospace,SFMono-Regular,Menlo,monospace;letter-spacing:.09em}.subjectRail i{display:block;height:1px;background:linear-gradient(90deg,rgba(213,184,109,.82),rgba(98,201,230,.42),transparent);box-shadow:0 0 18px rgba(213,184,109,.10)}
        .passportSpecimen{display:grid;grid-template-columns:minmax(350px,.78fr) minmax(0,1.22fr);overflow:hidden}.passportIntro{position:relative;display:flex;min-width:0;flex-direction:column;justify-content:center;padding:48px;border-right:1px solid rgba(213,184,109,.15);background:radial-gradient(circle at 20% 20%,rgba(213,184,109,.10),transparent 42%),linear-gradient(180deg,rgba(213,184,109,.018),transparent)}.passportIntro::after{content:"";position:absolute;right:26px;top:50%;width:1px;height:35%;transform:translateY(-50%);background:linear-gradient(180deg,transparent,rgba(213,184,109,.52),transparent);box-shadow:0 0 18px rgba(213,184,109,.18)}.passportIntro h2{max-width:100%;font-size:clamp(30px,3vw,42px);line-height:1.08;overflow-wrap:normal;word-break:normal;hyphens:none}.passportMark{display:flex;align-items:center;gap:11px;margin-top:32px;color:rgba(255,255,255,.42);font:600 10px/1.4 ui-monospace,SFMono-Regular,Menlo,monospace;letter-spacing:.12em}.passportMark b{color:var(--gold);font-size:17px;text-shadow:0 0 16px rgba(213,184,109,.38)}.passportLayers{padding:26px 36px 30px}.layersTitle{margin:0 0 10px;color:rgba(255,255,255,.52);font-size:13px}.passportLayer{display:grid;grid-template-columns:44px 1fr;gap:15px;align-items:start;padding:15px 0;border:0;border-bottom:1px solid rgba(255,255,255,.07);border-radius:0;background:none}.passportLayer:last-child{border-bottom:0}.passportLayer b{padding-top:3px;color:rgba(98,201,230,.92);text-shadow:0 0 15px rgba(98,201,230,.24)}.passportLayer h3{margin:0 0 4px;font:500 17px/1.25 Georgia,serif}.passportLayer p{margin:0;font-size:13px;line-height:1.52}
        .entryScene{padding:44px;background:radial-gradient(circle at 6% 12%,rgba(213,184,109,.055),transparent 28%),linear-gradient(145deg,rgba(255,255,255,.012),rgba(255,255,255,.005))}.entryHeading{max-width:760px}.entryModes{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-top:26px}.entryMode{position:relative;min-height:190px;padding:24px 22px;border-color:rgba(213,184,109,.13);background:linear-gradient(145deg,rgba(213,184,109,.04),rgba(98,201,230,.02))}.entryMode b{color:rgba(213,184,109,.66)}.entryMode h3{margin-top:26px;font:500 19px/1.25 Georgia,serif}.entryMode p{font-size:13px}.entryMode::after{content:"";position:absolute;left:22px;right:22px;bottom:18px;height:1px;background:linear-gradient(90deg,rgba(213,184,109,.52),transparent)}
        .freyContextScene{display:grid;grid-template-columns:minmax(0,1fr) minmax(340px,.92fr);gap:34px;align-items:center;padding:38px}.freyContextScene .contextGrid{grid-template-columns:repeat(2,1fr)}.contextEmpty{display:grid;grid-template-columns:auto 1fr auto 1fr auto;align-items:center;gap:9px;color:rgba(255,255,255,.42);font:600 9px/1.4 ui-monospace,SFMono-Regular,Menlo,monospace;letter-spacing:.12em}.contextEmpty i{height:1px;background:linear-gradient(90deg,rgba(213,184,109,.30),rgba(98,201,230,.20))}
        .guideBridge{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:34px;align-items:center;padding:42px;background:radial-gradient(circle at 84% 50%,rgba(98,201,230,.055),transparent 30%),linear-gradient(120deg,rgba(98,201,230,.035),rgba(213,184,109,.028))}.guideBridge>div:first-child{max-width:730px}.guideLinks{display:grid;gap:11px;min-width:220px}.methodCta{display:inline-flex;align-items:center;justify-content:center;min-height:46px;padding:0 20px;border:1px solid rgba(213,184,109,.42);border-radius:999px;background:rgba(213,184,109,.025);color:#e7ca7c!important;font:700 11px/1 ui-monospace,SFMono-Regular,Menlo,monospace;letter-spacing:.08em;text-transform:uppercase;box-shadow:inset 0 0 18px rgba(213,184,109,.025)}.methodCta:hover{border-color:rgba(213,184,109,.72);background:rgba(213,184,109,.055)}.guideLinks .freyTextLink{text-align:center}
        .applicationScene{display:grid;grid-template-columns:minmax(0,1fr) minmax(360px,.88fr);overflow:hidden;border-color:rgba(213,184,109,.14);background:radial-gradient(circle at 14% 34%,rgba(213,184,109,.055),transparent 30%),linear-gradient(145deg,rgba(255,255,255,.014),rgba(255,255,255,.006))}.applicationCopy{display:flex;flex-direction:column;justify-content:center;padding:48px}.applicationCta{position:relative;display:inline-flex;align-items:center;justify-content:center;gap:24px;min-width:220px;min-height:50px;margin-top:14px;padding:0 22px;border:1px solid rgba(226,190,91,.64);border-radius:999px;background:linear-gradient(90deg,rgba(128,91,20,.46),rgba(213,184,109,.20),rgba(104,74,19,.42));color:#f4db8c!important;font:800 11px/1 ui-monospace,SFMono-Regular,Menlo,monospace;letter-spacing:.10em;text-transform:uppercase;box-shadow:inset 0 0 20px rgba(246,214,125,.05),0 0 18px rgba(213,184,109,.10)}.applicationCta::before{content:"✦";margin-right:-10px;color:#f2ce70;text-shadow:0 0 14px rgba(242,206,112,.65)}.applicationCta:hover{transform:translateY(-1px);border-color:rgba(245,210,115,.84);box-shadow:inset 0 0 22px rgba(246,214,125,.08),0 0 28px rgba(213,184,109,.18)}.requestSheet{padding:30px 32px;border-left:1px solid rgba(213,184,109,.14);background:linear-gradient(180deg,rgba(0,0,0,.10),rgba(0,0,0,.18))}.requestSheetTop{margin-bottom:20px}.requestField{padding:14px 0;border-bottom:1px solid rgba(255,255,255,.07)}.requestField span{display:block;color:rgba(255,255,255,.56);font-size:11px;line-height:1.45}.requestField i{display:block;width:72%;height:1px;margin-top:13px;background:linear-gradient(90deg,rgba(213,184,109,.48),rgba(98,201,230,.16),transparent)}
        .trustLayer{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:0;overflow:hidden}.trustLayer>div{min-width:0;padding:36px}.trustLayer>div+div{border-left:1px solid rgba(255,255,255,.07)}.trustLayer h2{font-size:clamp(25px,3vw,34px)}.trustLayer h2,.trustLayer p,.trustLayer li{overflow-wrap:anywhere;word-break:break-word}.trustLayer li,.trustLayer p{font-size:13px}.processScene{padding:38px}.processScene h2{font-size:clamp(30px,3.5vw,42px)}.freyFinal{margin-top:18px;border-radius:22px;border-color:rgba(213,184,109,.18);background:radial-gradient(circle at 82% 50%,rgba(213,184,109,.08),transparent 28%),linear-gradient(120deg,rgba(213,184,109,.05),rgba(98,201,230,.025))}.freyFinal .applicationCta{margin:0}
        @keyframes phiPulse{0%,100%{box-shadow:0 0 0 13px rgba(213,184,109,.035),0 0 42px rgba(226,185,74,.22),0 0 82px rgba(226,185,74,.10)}50%{box-shadow:0 0 0 15px rgba(213,184,109,.05),0 0 58px rgba(226,185,74,.34),0 0 105px rgba(226,185,74,.16)}}
        @media(prefers-reduced-motion:reduce){.fieldCore{animation:none}.readingCta,.applicationCta,.methodCta{transition:none}}
