...
export default function PhiBtcTimingWindowsFounding({
  ...
  const t = ru ? {
    ...
    now: "NOW",
    future: "FUTURE",
    pastMeaning: "Что было зафиксировано в начале",
    pastInternal: "Locked baseline",
    baselineFormat: "FORMAT ONLY · BOUND AFTER BASELINE RUN",
    baselineBinding: "В активном объекте здесь находится persisted BASELINE run. Public offer не подменяет его текущим NOW.",
    nowMeaning: "Что evidence говорит сейчас",
    nowInternal: "Current accepted BTC field",
    evidenceStatus: "EVIDENCE STATUS",
    futureMeaning: "Что мы наблюдаем впереди",
    futureInternal: "Prospective window",
    windowFormat: "FORMAT ONLY · BTC-DERIVED WINDOW",
    windowBinding: "Точная дата появится только из accepted BTC window в BASELINE или checkpoint run. Astro окно не создаёт.",
    ...
    now: "NOW",
    future: "FUTURE",
    pastMeaning: "What was fixed at the start",
    pastInternal: "Locked baseline",
    baselineFormat: "FORMAT ONLY · BOUND AFTER BASELINE RUN",
    baselineBinding: "In an active object this position binds the persisted BASELINE run. The public offer does not substitute mutable NOW data.",
    nowMeaning: "What the evidence says now",
    nowInternal: "Current accepted BTC field",
    evidenceStatus: "EVIDENCE STATUS",
    futureMeaning: "What we are watching ahead",
    futureInternal: "Prospective window",
    windowFormat: "FORMAT ONLY · BTC-DERIVED WINDOW",
  ...
  return <>
              ...
              <div className="activeHorizon">
                ...
                <div className="futureField" aria-label={`${t.future}: ${t.futureMeaning}`}>
                  <article className="windowField">
                    ...
                    <b>DATE RANGE · FROM ACCEPTED BTC WINDOW</b>
                    <p>{t.windowBinding}</p>
                  </article>
    ...
    <style jsx>{`
      ...
      .cosmographBody{display:grid;grid-template-columns:minmax(0,2.618fr) minmax(240px,1fr)}.primaryField{padding:34px;border-right:1px solid #46433c}.timeAxisLabels{display:grid;grid-template-columns:27.6% 27.6% 44.8%;font-size:12px;letter-spacing:.16em;color:#aaa59b;margin-bottom:13px}.timeAxisLabels span:nth-child(2){text-align:center;color:#d8ff72}.timeAxisLabels span:last-child{text-align:right}
